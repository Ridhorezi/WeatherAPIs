...
public class HourlyWeatherApiControllerTests {
	...
	public void testGetByIpShouldReturn400BadRequestBecauseNoHeaderXCurrentHour() throws Exception {
		...
		mockMvc.perform(get(END_POINT_PATH)).andExpectAll(status().isBadRequest()).andDo(print());
	}
	...
	public void testGetByIpShouldReturn200OK() throws Exception {
		...
		HourlyWeather forecast1 = new HourlyWeather().location(location).hourOfDay(8).temperature(10).precipitation(70)
				.status("Sunny");

		HourlyWeather forecast2 = new HourlyWeather().location(location).hourOfDay(9).temperature(10).precipitation(70)
				.status("Sunny");
		...
		mockMvc.perform(get(END_POINT_PATH).header(X_CURRENT_HOUR, String.valueOf(currentHour)))
				...
				.andExpect(jsonPath("$.hourly_forecast[0].hour_of_day", is(8))).andDo(print());
	}
	...
	public void testGetByCodeShouldReturn400BadRequest() throws Exception {
		...
		mockMvc.perform(get(requestURI)).andExpectAll(status().isBadRequest()).andDo(print());
	}
	...
	public void testGetByCodeShouldReturn404NotFound() throws Exception {
		...
		mockMvc.perform(get(requestURI).header(X_CURRENT_HOUR, String.valueOf(currentHour)))
				.andExpectAll(status().isNotFound()).andDo(print());
	}
	...
	public void testUpdateShouldReturn400BadRequestBecauseNoData() throws Exception {
		...
		String requestURI = END_POINT_PATH + "/JKT";
	...
	public void testUpdateShouldReturn400BadRequestBecauseInvalidData() throws Exception {
		...
		HourlyWeatherDTO dto1 = new HourlyWeatherDTO().hourOfDay(8).temperature(133).precipitation(70).status("Cloudy");

		HourlyWeatherDTO dto2 = new HourlyWeatherDTO().hourOfDay(9).temperature(133).precipitation(70).status("Sunny");
		...
		mockMvc.perform(put(requestURI).contentType(MediaType.APPLICATION_JSON).content(requestBody))
				.andExpect(status().isBadRequest())
				.andExpect(jsonPath("$.errors[0]", containsString("Temprature must be in the range"))).andDo(print());
	}

	@Test
	public void testUpdateShouldReturn404() throws Exception {

		String locationCode = "NOT_FOUND";

		String requestURI = END_POINT_PATH + "/" + locationCode;

		HourlyWeatherDTO dto1 = new HourlyWeatherDTO().hourOfDay(9).temperature(10).precipitation(70).status("Sunny");
	...
	public void testUpdateShouldReturn200OK() throws Exception {
		...
		HourlyWeatherDTO dto1 = new HourlyWeatherDTO().hourOfDay(8).temperature(10).precipitation(70).status("Sunny");

		HourlyWeatherDTO dto2 = new HourlyWeatherDTO().hourOfDay(9).temperature(10).precipitation(70).status("Sunny");
